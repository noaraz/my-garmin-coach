...
- **Garmin calendar read-back works via calendar-service** — `GET /calendar-service/year/{year}/month/{month}` returns `calendarItems` with `{workoutId, date, title}`. The old `/workout-service/schedule/{id}` endpoint returns 404 always (do not use). Calendar-based reconciliation uses the calendar-service endpoint to compare `(workoutId, date)` pairs.
