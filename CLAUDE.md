...
- **`_is_garmin_404(exc)`** — typed helper in `sync.py`. Three-tier detection: (1) `GarthHTTPError` → `exc.error.response.status_code == 404`; (2) curl_cffi `HTTPError` (not wrapped by garth) → `exc.response.status_code == 404`; (3) string fallback `"404" in str(exc)` for `requests.exceptions.HTTPError` raised without a response object. Test factory for GarthHTTPError: `GarthHTTPError(msg="...", error=RequestsHTTPError(response=resp))` where `resp.status_code = 404`. Test factory for curl_cffi path: inner class with `response.status_code = 404`.
- **Garmin calendar read-back works via calendar-service** — `GET /calendar-service/year/{year}/month/{month}` returns `calendarItems` with `{workoutId, date, title}`. **Garmin uses 0-indexed months** (Jan=0, Dec=11) — the adapter converts internally from Python's 1-indexed `date.month`. The old `/workout-service/schedule/{id}` endpoint returns 404 always (do not use).
- **Unschedule a calendar entry**: `DELETE /workout-service/schedule/{schedule_id}` removes a single calendar entry without deleting the workout template. Each calendar item has a unique `id` field (the schedule entry ID). Used for duplicate cleanup.
- **Calendar month views overlap** — each Garmin calendar "month" returns items spanning ~5 weeks (partial weeks from adjacent months). When fetching multiple months, deduplicate calendar items by `id` before processing to avoid double-counting.
- **Dedup module**: `backend/src/garmin/dedup.py` — `find_matching_garmin_workout(name, garmin_workouts)` matches by name (case-insensitive). `find_orphaned_garmin_workouts()` finds untracked Garmin workouts safe to delete. `find_unscheduled_workouts()` compares DB `(workoutId, date)` pairs against Garmin calendar items for reconciliation. `find_duplicate_calendar_entries()` detects duplicate `(workoutId, date)` pairs on the calendar and returns extra schedule IDs to delete.
...
- **sync_all reconciliation (calendar-based)**: Fetches Garmin calendar via `GET /calendar-service/year/{year}/month/{month}` to get `(workoutId, date)` pairs. Compares against DB workouts with `sync_status="synced"` + `completed=False`. Two reconciliation paths: (1) template still exists on Garmin but not scheduled on correct date → `reschedule_workout()` (cheap, no re-push). (2) Template also gone from Garmin → mark as `sync_status="modified"` for full re-push. Pure function: `find_unscheduled_workouts()` in `dedup.py`. `SyncAllResponse` includes `rescheduled: int`. Logged as `"Reconciliation: N workouts rescheduled, M re-queued"`.
...
## Garmin Auth Version Feature Flag (added 2026-04-14, refactored 2026-04-16)

- **Runtime toggle**: `SystemConfig` table stores `garmin_auth_version` ("v1" | "v2")
- **Admin endpoint**: `POST /api/v1/admin/garmin-auth-version` to switch at runtime — also calls `client_cache.clear()` so cached adapters from the old version don't bypass the version-mismatch check for up to 1h TTL.
...
- **Version mismatch → disconnect**: `_get_garmin_adapter` in `sync.py` compares `profile.garmin_auth_version` against the global `SystemConfig` flag. If they differ, `clear_garmin_connection(..., keep_credentials=True)` clears the token + sets `garmin_connected=False` but keeps credentials so auto-reconnect can retry with the new version. Returns 403 with "Please reconnect in Settings."
...
- **Central enum + helpers** (`src/garmin/auth_version.py`): `GarminAuthVersion(StrEnum)` replaces `"v1"/"v2"` string literals — StrEnum gives `==` string equality so raw strings still work at call sites (use `==`, not `is`). `parse(value)` defensively coerces from DB/env. `get_db_auth_version(session)` reads `SystemConfig` (use this instead of inlining `session.get(SystemConfig, SYSTEM_CONFIG_KEY)`).
- **Shared disconnect** (`src/garmin/disconnect.py`): `clear_garmin_connection(profile, user_id, session, *, keep_credentials=False)` is the single disconnect path — clears token, sets `garmin_connected=False`, invalidates caches. `keep_credentials=True` for version-mismatch; `False` for user-initiated disconnect.
